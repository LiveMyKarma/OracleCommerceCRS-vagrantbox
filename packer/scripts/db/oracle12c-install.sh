...
sudo umount tmpfs
sudo mount -t tmpfs shmfs -o size=3g /dev/shm

sudo sed -i 's/.*\/dev\/shm.*/shmfs      /dev/shm    tmpfs   size=3g        0 0/g' /etc/fstab
sudo sed -i 's/.*\/dev\/shm.*/mount /dev/shm >/dev/null 2>&1/g' /etc/rc.d/rc.sysinit
